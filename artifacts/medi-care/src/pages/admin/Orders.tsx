...
export default function AdminOrders() {
  ...
  const load = () =>
    api.getOrders(filter ? { status: filter } : undefined).then(r => {
      setOrders(r.orders);
      setLoading(false);
    });
  ...
  const updateStatus = async (id: number, status: string) => {
    setUpdating(id);
    try {
      // When marking delivered, auto-mark payment paid for COD orders
  ...
  const markPaid = async (order: any) => {
    const orderNum = `#FM-${String(order.id).padStart(4, "0")}`;
    if (!confirm(`Confirm UPI payment received for Order ${orderNum} (₹${Number(order.totalAmount).toFixed(2)})?`)) return;
        ...
        <div className="space-y-4">
          {orders.length === 0 ? (
            ...
            <div key={order.id} className="bg-white rounded-xl border border-gray-100 p-4">
              ...
              {/* Footer: payment info + status selector + mark paid */}
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2 text-xs flex-wrap">
                  <span className={`px-2 py-0.5 rounded font-medium ${order.paymentMethod === "upi" ? "bg-teal-100 text-teal-700" : "bg-gray-100 text-gray-600"}`}>
                    {order.paymentMethod === "upi" ? "UPI" : "Cash on Delivery"}
                  </span>
                  <span className={`px-2 py-0.5 rounded font-medium capitalize ${order.paymentStatus === "paid" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                ...
                <div className="flex items-center gap-2">
                  {/* Mark Paid button — only for UPI orders not yet paid */}
                  {order.paymentMethod === "upi" && order.paymentStatus !== "paid" && (
                    <button
                      onClick={() => markPaid(order)}
                      disabled={markingPaid === order.id}
                      className="flex items-center gap-1.5 bg-green-500 hover:bg-green-600 disabled:opacity-60 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition"
                    >
              ...
              {/* UPI payment pending reminder */}
              {order.paymentMethod === "upi" && order.paymentStatus !== "paid" && order.status !== "cancelled" && (
