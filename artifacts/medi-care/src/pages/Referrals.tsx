      ...
      <div className="max-w-lg mx-auto">
          ...
          {[
            {
              icon: IndianRupee,
              label: "Credits",
              value: `₹${Number(info.credits).toFixed(0)}`,
              color: "text-teal-600",
            },
            {
              icon: Users,
        ...
        <div className="bg-white rounded-xl border border-teal-200 p-5 mb-4">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Share this code with friends. Both of you earn ₹50 when they
            register!
          </p>
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-teal-50 border border-teal-200 rounded-lg px-4 py-3">
              <span className="font-mono font-bold text-teal-600 text-xl tracking-widest">
                {info.referralCode}
              </span>
            </div>
            <button
              onClick={copy}
              className={`px-4 py-3 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-all ${copied ? "bg-green-500 text-white" : "bg-teal-500 text-white hover:bg-teal-600"}`}
            >
              <Copy size={14} />
              {copied ? "Copied!" : "Copy"}
            </button>
        ...
        {!info.hasUsedReferral && (
          <div className="bg-white rounded-xl border border-gray-100 p-5 mb-4">
            ...
            <p className="text-xs text-gray-500 mb-3">
              Enter a friend's code to earn ₹50 credits.
            </p>
            <form onSubmit={apply} className="flex gap-2">
              ...
              <button
                type="submit"
                className="bg-teal-500 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-teal-600"
              >
                Apply
              </button>
            </form>
