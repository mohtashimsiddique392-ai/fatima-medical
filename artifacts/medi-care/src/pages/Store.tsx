...
interface Product { id: number; name: string; description?: string; price: string; category: string; stock: number; dosage?: string; howToTake?: string; requiresPrescription: boolean; imageUrl?: string; }

const CAT_COLORS: Record<string, string> = { "Pain Relief": "bg-red-100 text-red-700", "Antibiotic": "bg-yellow-100 text-yellow-700", "Supplements": "bg-green-100 text-green-700", "Allergy": "bg-purple-100 text-purple-700", "Gastro": "bg-orange-100 text-orange-700", "Diabetes": "bg-blue-100 text-blue-700" };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-teal-100 px-4 py-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-xl font-bold text-gray-900">Welcome to Fatima Medical</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        {loading ? (
          <div className="grid md:grid-cols-3 gap-4">{[...Array(6)].map((_, i) => <div key={i} className="bg-white rounded-xl p-4 h-40 animate-pulse border border-gray-100" />)}</div>
        ) : products.length === 0 ? (
          <div className="text-center py-20 text-gray-400">No medicines found.</div>
        ) : (
          <div className="grid md:grid-cols-3 gap-4">
            {products.map(p => (
              <div key={p.id} className="bg-white rounded-xl border border-gray-100 p-4 hover:border-teal-200 hover:shadow-sm transition-all">
                ...
                <p className={`text-xs mb-3 font-medium ${p.stock <= 10 ? "text-orange-500" : "text-gray-400"}`}>{p.stock <= 10 ? `${p.stock} In Stock` : `${p.stock} in stock`}</p>
                {p.howToTake && <button onClick={() => setSelected(p)} className="text-xs text-teal-600 hover:underline mb-2 block">How to take →</button>}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => { handleAdd(selected); setSelected(null); }} className="flex-1 bg-teal-500 text-white py-2.5 rounded-lg text-sm font-medium">Add to Cart</button>
              <button onClick={() => setSelected(null)} className="flex-1 border border-gray-200 py-2.5 rounded-lg text-sm text-gray-600">Close</button>
            </div>
          </div>
        </div>
      )}
