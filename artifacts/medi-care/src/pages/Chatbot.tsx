...
const QUICK = ["How to take Paracetamol?", "I have fever", "Do you deliver?", "Payment options?", "My referral code"];

export default function Chatbot() {
  const { user, addToCart } = useAuth();
  const [messages, setMessages] = useState<Message[]>([
    { role: "bot", text: "Hello! Welcome to Fatima Medical. I'm your pharmacy assistant. I can help with medicine dosage, how to take medicines, delivery info, and more. What can I help you with today?" }
  ]);
  ...
  const send = async (text: string) => {
    ...
    setLoading(true);
    try {
      const res = await api.chat({ message: text, customerId: user?.id });
      setMessages(prev => [...prev, { role: "bot", text: res.reply, products: res.suggestedProducts }]);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-teal-100 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold text-gray-900 text-sm">Fatima Medical Assistant</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {loading && (
          <div className="flex gap-2">
            <div className="w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center"><Bot size={16} className="text-teal-600" /></div>
      ...
      <div className="px-4 py-2 flex gap-2 overflow-x-auto bg-white border-t border-gray-100">
        {QUICK.map(q => (
          <button key={q} onClick={() => send(q)} className="flex-shrink-0 text-xs bg-teal-50 text-teal-700 border border-teal-200 px-3 py-1.5 rounded-full hover:bg-teal-100">{q}</button>
        ))}
      </div>
      ...
      <div className="bg-white border-t border-gray-100 px-4 py-3 flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && send(input)}
          placeholder="Ask about medicines, delivery, payment..."
