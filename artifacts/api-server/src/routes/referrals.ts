...
router.post("/apply", async (req, res) => {
  ...
  if (!customerId || !referralCode) return res.status(400).json({ error: "customerId and referralCode required" });
  ...
  await db.update(customersTable).set({ referredBy: referrer.id, referralCredits: String(Number(customer.referralCredits) + 50) }).where(eq(customersTable.id, customer.id));
  await db.update(customersTable).set({ referralCredits: String(Number(referrer.referralCredits) + 50) }).where(eq(customersTable.id, referrer.id));

  res.json({ message: "Referral applied! You and your referrer each earned ₹50 credits." });
});
