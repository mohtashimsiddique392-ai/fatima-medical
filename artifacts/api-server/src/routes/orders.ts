...
router.get("/", async (req, res) => {
  const { customerId, status } = req.query as {
    customerId?: string;
    status?: string;
  };
...
router.post("/", async (req, res) => {
  ...
  for (const item of enrichedItems) {
    await db
      ...
      .values({ orderId: order.id, ...item, price: String(item.price) });
    await db
  ...
  if (creditsUsed > 0) {
    await db
      ...
      .set({
        referralCredits: String(Number(customer.referralCredits) - creditsUsed),
      })
      .where(eq(customersTable.id, customerId));
  }
  ...
  if (status === "delivered") {
    ...
    if (customer?.referredBy) {
      ...
      await db
        .update(customersTable)
        .set({
          referralCredits: String(Number(customer.referralCredits) + bonus),
        })
